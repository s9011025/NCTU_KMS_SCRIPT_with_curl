...
echo 
echo 連線認證網頁
curl -d "mailaccount=%NCTU_USER%&mailserver=nctu.edu.tw&password=%NCTU_PASS%&Submit=送出" https://kmserv.nctu.edu.tw/login-email.php >%TEMP%\office_out.txt
find "ff0000" %TEMP%\office_out.txt
del %TEMP%\office_out.txt
echo 認證完畢後按任意鍵繼續
pause
